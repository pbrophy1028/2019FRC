...
public class PixyI2C extends Subsystem {
  PixyPacketLine values;
	I2C pixy;
	Port port = Port.kOnboard;
	PixyPacketLine[] packets;
	PixyException pExc;
	String print;
	public PixyI2C() {
...
//		values = new PixyPacket();
	...
	public PixyI2C(I2C argPixy, PixyPacketLine[] argPixyPacket, PixyException argPixyException, PixyPacketLine argValues){
		pixy = argPixy;
	...
	*/
	...
	public PixyPacketLine readPacket(int Signature) throws PixyException { //The signature should be which number object in 
		...
		for (int i = 0; i <= 16; i++) {
			...
			if (syncWord == 0xaa55) { //Check is first 2 bytes equal a "sync word", which indicates the start of a packet of valid data
				...
				packets[Sig - 1] = new PixyPacketLine();
				packets[Sig - 1].x1 = cvt(rawData[i+9], rawData[i+8]);
				packets[Sig - 1].y1 = cvt(rawData[i+11], rawData[i+10]);
				packets[Sig - 1].x2 = cvt(rawData[i+13], rawData[i+12]);
				packets[Sig - 1].y2 = cvt(rawData[i+15], rawData[i+14]);
				//Checks whether the data is valid using the checksum *This if block should never be entered*
				if (Checksum != Sig + packets[Sig - 1].x1 + packets[Sig - 1].y1 + packets[Sig - 1].x2 + packets[Sig - 1].y2) {
					packets[Sig - 1] = null;
					throw pExc;
				}
				break;
		...
		PixyPacketLine pkt = packets[Signature - 1];
