 ...
 * Add your docs here.
 */
public class PixySPI extends Subsystem {
  // Put methods for controlling this subsystem
  // here. Call these from Commands.
  PixyPacketLine values = null;
	SPI pixy = null;
	Port port = null;
	HashMap<Integer, ArrayList<PixyPacketLine>> packets = null;
	...
	public PixySPI(SPI argPixy, HashMap<Integer, ArrayList<PixyPacketLine>> argPixyPacket, PixyException argPixyException){
		pixy = argPixy;
	...
	public int readPackets() throws PixyException { //The signature should be which number object in 
		...
		for(int i=1; i<=PIXY_SIG_COUNT; i++) {
			packets.put(i, new ArrayList<PixyPacketLine>());
		}
		...
		if(numBlocks > 0) {
			...
			for(int i=0; i < numBlocks; i++) {
				...
				PixyPacketLine packet = new PixyPacketLine();
				int signature = blocks.get(i)[0];
				packet.x1 = blocks.get(i)[1];
				packet.y1 = blocks.get(i)[2];
				packet.x2 = blocks.get(i)[3];
				packet.y2 = blocks.get(i)[4];

				if(debug >= 1) {SmartDashboard.putString("Pixy readPackets: Signature: " + Integer.toString(signature), Integer.toString(signature));}
				if(debug >= 1) {SmartDashboard.putString("Pixy readPackets: " + Integer.toString(signature) + ": X: ", Integer.toString(packet.x1));}
				if(debug >= 1) {SmartDashboard.putString("Pixy readPackets: " + Integer.toString(signature) + ": Y: ", Integer.toString(packet.y1));}
				if(debug >= 1) {SmartDashboard.putString("Pixy readPackets: " + Integer.toString(signature) + ": Width: ", Integer.toString(packet.x2));}
				if(debug >= 1) {SmartDashboard.putString("Pixy readPackets: " + Integer.toString(signature) + ": Height: ", Integer.toString(packet.y2));}
				if(debug >= 2) {logger.log(Level.INFO, "Pixy readPackets: Signature: " + Integer.toString(signature), Integer.toString(signature));}
				if(debug >= 2) {logger.log(Level.INFO, "Pixy readPackets: " + Integer.toString(signature) + ": X: ", Integer.toString(packet.y1));}
				if(debug >= 2) {logger.log(Level.INFO, "Pixy readPackets: " + Integer.toString(signature) + ": Y: ", Integer.toString(packet.x2));}
				if(debug >= 2) {logger.log(Level.INFO, "Pixy readPackets: " + Integer.toString(signature) + ": Width: ", Integer.toString(packet.x2));}
				if(debug >= 2) {logger.log(Level.INFO, "Pixy readPackets: " + Integer.toString(signature) + ": Height: ", Integer.toString(packet.y2));}
